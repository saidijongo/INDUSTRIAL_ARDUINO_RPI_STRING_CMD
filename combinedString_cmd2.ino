...
void runPumps(int pumpNumber, int runTime) {
  ...
  // Turn off the pump after the run time has elapsed
  digitalWrite(motorPins[pumpNumber], LOW);
  Serial.println("Stopping pump " + String(pumpNumber));
}
*/
unsigned long referenceStartTime = 0;

void runPumps(int pumpNumber, int runTime) {
  // Record the start time only for the first pump
  if (pumpNumber == 0) {
    Serial.println("Running pumps for " + String(runTime) + " milliseconds");
    referenceStartTime = millis();
  }

  // Check if the reference start time is set
  if (referenceStartTime > 0) {
    // Calculate the elapsed time for the current pump
    unsigned long elapsedTime = millis() - referenceStartTime;

    // Check if the pump should still be running
    if (elapsedTime <= runTime) {
      digitalWrite(motorPins[pumpNumber], HIGH);
      Serial.println("Running pump " + String(pumpNumber));
    } else {
      // Turn off the pump after the run time has elapsed
      digitalWrite(motorPins[pumpNumber], LOW);
      Serial.println("Stopping pump " + String(pumpNumber));
...
// Add a function to stop all pumps
void stopPumps() {
  for (int i = 0; i < numPumps; i++) {
    digitalWrite(motorPins[i], LOW);
  }

  // Reset the reference start time
    ...
    for (int i = 0; i <= NUM_LEDS - GROUP_SIZE; i++) {
      ...
      if (Serial.available() > 0) {
        ...
        if (data.indexOf("adiosMf,0") != -1) {
          stopPumps();  // Stop pumps when "adiosMf,0" is received
          FastLED.clear();
          FastLED.show();
          return;
        }
