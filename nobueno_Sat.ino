          ...
          if (nextBracketIndex != -1 && endIndex != -1) {
            ...
            if (commaIndex != -1) {
              ...
              if (commandType == "PUMPMOTOR_OPERATION" ||
                ...
                // Update maxRunTime if param2 is greater than current maxRunTime
                if (param2.toInt() > maxRunTime) {
                  maxRunTime = param2.toInt();
                }
...
void loop() {
  for (int i = 0; i < numPumps; i++) {
    if (millis() >= maxRunTime && digitalRead(motorPins[i]) == HIGH) {
      digitalWrite(motorPins[i], LOW);
    }
  }
